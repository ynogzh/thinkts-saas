...
export default class Payment_orderService extends BaseService {
  async createPay(opts, think) {
  ...
  @Params({ tenant_id: "int", pay_no: "int" })
  async getByPayNo(opts) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
  ...
  async compensatePaid(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
