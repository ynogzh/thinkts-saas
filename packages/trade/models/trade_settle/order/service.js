...
export async function markProcessing(opts) {
...
export async function payoutRecords(opts) {
  if (!opts?.tenant_id) throw new Error("tenant_id is required");
...
export async function reconciliationOverview(opts) {
  if (!opts?.tenant_id) throw new Error("tenant_id is required");
  const tenant_id = Number(opts.tenant_id);
