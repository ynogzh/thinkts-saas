...
export default class CouponService extends BaseService {
  ...
  async issueFromTemplate(opts, think) {
  ...
  @Params({ tenant_id: "int", user_coupon_id: "int" })
  async calculateDiscount(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
  ...
  @Params({ tenant_id: "int", user_coupon_id: "int", biz_type: "int", biz_id: "int" })
  async useCoupon(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    if (!opts?.user_coupon_id) throw new Error("user_coupon_id is required");
