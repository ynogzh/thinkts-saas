...
export function ResourceFormField({
  ...
  return (
    <div className='grid grid-cols-6 items-center gap-x-2 gap-y-1'>
      <span className='col-span-2 text-end text-sm text-muted-foreground'>{field.label}</span>
            ...
            {mode !== 'view' && onUpload && (
              <Input
                ...
                onChange={async (e) => {
                  ...
                  await onUpload(field, file); e.currentTarget.value = ''
                }}
              />
            )}
          </div>
