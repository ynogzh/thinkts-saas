...
export default class ShareService extends BaseService {
  @Params({ tenant_id: "int", session_id: "int", trade_order_id: "int", merchant_id: "int" })
  async createSessionShares(opts, think) {
  ...
  @Params({ tenant_id: "int" })
  async restoreBySettleOrder(opts) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    if (opts?.settle_order_id === undefined) throw new Error("settle_order_id is required");
    const tenant_id = Number(opts.tenant_id);
    const settle_order_id = Number(opts.settle_order_id);
  ...
  async createSettlement(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
  ...
  @Params({ tenant_id: "int" })
  async createBatchSettlement(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    const tenant_id = Number(opts.tenant_id);
  ...
  async reverseSessionShares(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    if (!opts?.session_id) throw new Error("session_id is required");
    const tenant_id = Number(opts.tenant_id);
    const model = this.currentModel();
