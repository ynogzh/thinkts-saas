...
export default class DeviceService extends BaseService {
  ...
  @Params({ tenant_id: "int", device_id: "int", template: "int" })
  async applyParamTemplate(opts) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
  ...
  @Params({ tenant_id: "int", merchant_id: "int", type_id: "int", name: "int" })
  async registerDevice(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
  ...
  @Params({ tenant_id: "int", device_id: "int" })
  async restoreDeviceOnline(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    if (!opts?.device_id) throw new Error("device_id is required");
    const tenant_id = Number(opts.tenant_id);
  ...
  async resolveDeviceAlert(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    const tenant_id = Number(opts.tenant_id);
    const result = {
      device: null,
  ...
  async runHeartbeatSweep(opts, think) {
  ...
  async markStaleOffline(opts, think) {
  ...
  @Params({ tenant_id: "int", device_id: "int" })
  async issueStartCommand(opts, think) {
    if (!opts?.tenant_id) throw new Error("tenant_id is required");
    if (!opts?.device_id) throw new Error("device_id is required");
    const tenant_id = Number(opts.tenant_id);
